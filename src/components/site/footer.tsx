      ...
      <div className="container mx-auto px-4 py-16">
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-10">
          <div className="lg:col-span-2">
            ...
            <p className="mt-4 text-sm text-muted-foreground max-w-sm">India's premium AI-powered marketplace for certified pre-owned cars. Buy, sell, auction and compare with confidence.</p>
            <div className="flex items-center gap-3 mt-6">
              {[Facebook, Instagram, Twitter, Youtube].map((Icon, i) => (
                <a key={i} href="#" className="w-9 h-9 rounded-lg glass flex items-center justify-center hover:text-primary hover:border-primary/40 transition-colors"><Icon className="w-4 h-4" /></a>
              ))}
            </div>
          </div>
          {[
            { title: "Marketplace", links: [["Buy used cars", "/buy"], ["Sell your car", "/sell"], ["Auctions", "/auctions"], ["Compare cars", "/compare"], ["Dealers", "/dealers"]] },
            { title: "Company", links: [["About", "/about"], ["Blog", "/blog"], ["Contact", "/contact"], ["Careers", "/about"]] },
            { title: "Support", links: [["FAQ", "/faq"], ["Pricing", "/pricing"], ["Privacy", "/privacy"], ["Terms", "/terms"]] },
          ].map((c) => (
            <div key={c.title}>
