              ...
              {items.map((i) => {
                ...
                return (
                  <Link
                    key={i.to}
                    to={i.to}
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition ${active ? "bg-primary/15 text-primary" : "text-muted-foreground hover:bg-secondary/60 hover:text-foreground"}`}
                  >
...
export function StatCard({
  ...
  return (
    <div className="gradient-card glass rounded-2xl p-5 relative overflow-hidden hover-lift">
      <div
        className={`absolute -right-10 -top-10 w-32 h-32 rounded-full bg-gradient-to-br ${tones[accent || "primary"]} blur-2xl opacity-50`}
      />
