    ...
    <DashboardShell title="John Doe" subtitle="Buyer account" items={items}>
      ...
      <div className="grid gap-4">
        {myCars.map((c) => (
          <div
            key={c.id}
            className="gradient-card glass rounded-2xl p-4 flex flex-col sm:flex-row gap-4 items-start sm:items-center"
          >
            <img
              src={c.image}
              alt={c.model}
              className="w-full sm:w-40 h-28 rounded-xl object-cover"
            />
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                {c.year} · {c.km.toLocaleString()} km · {c.city}
              </p>
            ...
            <div className="flex items-center gap-4 text-xs text-muted-foreground">
              ...
              <span className="px-2 py-0.5 rounded-md bg-success/15 text-success font-semibold">
                Active
              </span>
            </div>
