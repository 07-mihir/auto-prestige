        ...
        <div className="relative h-full flex flex-col justify-between p-12">
          ...
          <div>
            ...
            <p className="mt-3 text-muted-foreground max-w-sm">
              Join 1M+ buyers and dealers using India's most trusted used car marketplace.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center p-6 sm:p-12 relative">
        ...
        <div className="relative w-full max-w-md">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Sign in to manage your listings, wishlist and offers.
          </p>

          <div className="mt-8 glass-strong rounded-xl p-1 grid grid-cols-2 gap-1">
            <button
              onClick={() => setTab("email")}
              className={`py-2 rounded-lg text-sm font-medium transition ${tab === "email" ? "gradient-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
            >
              Email
            </button>
            <button
              onClick={() => setTab("otp")}
              className={`py-2 rounded-lg text-sm font-medium transition ${tab === "otp" ? "gradient-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
            >
              Phone OTP
            </button>
          </div>
...
function InputField({ icon: Icon, label, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { icon: React.ElementType; label: string }) {
  return (
    <label className="block">
      ...
      <div className="relative mt-1.5">
        ...
        <input
          {...props}
          className="w-full bg-input border border-border rounded-lg pl-10 pr-3 py-2.5 text-sm outline-none focus:border-primary transition"
        />
      </div>
    </label>
  );
