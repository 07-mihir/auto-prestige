...
import { LayoutDashboard, Car, Users, BarChart3, MessageCircle, Calendar, Settings, Crown, Plus, Edit, Eye, Trash2 } from "lucide-react";
  ...
  return (
    <DashboardShell title="Royal Auto Mart" subtitle="Verified dealer" items={items}>
      <div className="flex items-center justify-between mb-5">
        ...
        <Link to="/sell"><Button className="rounded-lg gradient-primary text-primary-foreground"><Plus className="w-4 h-4 mr-2" />Add car</Button></Link>
      </div>
          ...
          <tbody>
            {cars.map((c) => (
              <tr key={c.id} className="border-t border-border/40">
                ...
                <td className="p-4 hidden md:table-cell text-muted-foreground">{c.year} · {c.km.toLocaleString()} km</td>
                <td className="p-4 font-bold text-primary">{formatPrice(c.price)}</td>
                <td className="p-4 hidden lg:table-cell"><span className="flex items-center gap-1"><Eye className="w-3.5 h-3.5" />{Math.floor(Math.random() * 2000 + 200)}</span></td>
                <td className="p-4"><span className="px-2 py-0.5 rounded-md text-[11px] bg-success/15 text-success font-semibold">Live</span></td>
