  ...
  head: () => ({
    meta: [
      { title: "Sell your car — DriveX" },
      {
        name: "description",
        content:
          "List your car for free, get an AI valuation and up to 8 instant offers from verified dealers in 24 hours.",
      },
    ],
  }),
  component: SellPage,
    ...
    <div className="container mx-auto px-4 py-10">
      <div className="text-center max-w-2xl mx-auto mb-10">
        ...
        <p className="mt-3 text-muted-foreground">
          Free listing · Instant valuation · Up to 8 dealer offers
        </p>
      </div>
        ...
        <div className="gradient-card glass-strong rounded-2xl p-6 sm:p-8">
          ...
          <div className="flex items-center gap-2 mb-8">
            {steps.map((s, i) => (
              <div key={s} className="flex-1 flex items-center gap-2">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${i <= step ? "gradient-primary text-primary-foreground" : "bg-secondary text-muted-foreground"}`}
                >
                  {i < step ? <CheckCircle2 className="w-4 h-4" /> : i + 1}
                </div>
                <span
                  className={`text-xs font-medium hidden sm:inline ${i <= step ? "text-foreground" : "text-muted-foreground"}`}
                >
                  {s}
                </span>
                {i < steps.length - 1 && (
          ...
          {step === 0 && (
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <Field label="Model">
                <input className="input" placeholder="e.g. 5 Series" />
              </Field>
              <Field label="Variant">
                <input className="input" placeholder="e.g. 530i M Sport" />
              </Field>
              <Field label="Year">
              ...
              <Field label="KM driven">
                <input className="input" type="number" placeholder="18200" />
              </Field>
              <Field label="City">
                <select className="input">
                  {cities.map((c) => (
          ...
          {step === 1 && (
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <Field label="Accidental history">
                <select className="input">
                  <option>Non-accidental</option>
              ...
              <Field label="Service history">
                <select className="input">
                  <option>Full service available</option>
              ...
              <Field label="RC status">
                <select className="input">
                  <option>Original RC</option>
                  <option>Duplicate RC</option>
                </select>
              </Field>
              <Field label="Pollution certificate">
                <select className="input">
                  <option>Valid</option>
              ...
              <Field label="Color">
                <input className="input" placeholder="e.g. Mineral White" />
              </Field>
              ...
              <Field label="Upload photos">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[1, 2, 3, 4].map((i) => (
                    <div
                      key={i}
                      className="aspect-square rounded-xl border-2 border-dashed border-border hover:border-primary/40 hover:bg-primary/5 flex flex-col items-center justify-center cursor-pointer transition"
                    >
          ...
          <div className="gradient-card glass rounded-2xl p-5">
            ...
            <ul className="space-y-3 text-sm">
              {[
                ...
                "AI fair-price valuation",
                "Up to 8 dealer offers in 24h",
