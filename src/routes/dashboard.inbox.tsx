...
const threads = [
  {
    name: "Royal Auto Mart",
    car: "BMW 5 Series",
    last: "Sure, you can come tomorrow at 11am.",
    unread: 2,
  },
  {
    name: "Sundaram Motors",
    car: "Mercedes GLE",
    last: "We can negotiate the final price.",
    unread: 0,
  },
      ...
      <div className="grid md:grid-cols-[300px_1fr] gap-4 h-[600px]">
        <div className="gradient-card glass rounded-2xl p-3 overflow-y-auto">
          {threads.map((t, i) => (
            <button
              key={i}
              onClick={() => setActive(i)}
              className={`w-full text-left p-3 rounded-xl mb-1 transition ${active === i ? "bg-primary/15" : "hover:bg-secondary/60"}`}
            >
        ...
        <div className="gradient-card glass rounded-2xl p-5 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto space-y-3">
            <div className="max-w-[70%] glass rounded-2xl rounded-tl-sm p-3 text-sm">
              Hi! Is the car still available?
            </div>
            <div className="max-w-[70%] ml-auto gradient-primary text-primary-foreground rounded-2xl rounded-tr-sm p-3 text-sm">
              Yes, it is. Want to schedule a test drive?
            </div>
            <div className="max-w-[70%] glass rounded-2xl rounded-tl-sm p-3 text-sm">
              {threads[active].last}
            </div>
          ...
          <div className="flex gap-2 mt-4">
            <input
              className="flex-1 bg-secondary/50 border border-border rounded-lg px-3 py-2 text-sm outline-none focus:border-primary"
