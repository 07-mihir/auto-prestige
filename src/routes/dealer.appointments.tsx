...
import { LayoutDashboard, Car, Users, BarChart3, MessageCircle, Calendar, Settings, Crown, Clock, MapPin } from "lucide-react";
...
const appts = [
  { customer: "Aarav Sharma", car: "BMW 5 Series 530i", date: "Tue, May 12 · 11:00 AM", loc: "Showroom — Mumbai", status: "Confirmed" },
  { customer: "Priya Nair", car: "Mercedes GLE 300d", date: "Wed, May 13 · 4:00 PM", loc: "Home pickup — Bandra", status: "Pending" },
  { customer: "Vikas Iyer", car: "Hyundai Ioniq 5", date: "Fri, May 15 · 10:30 AM", loc: "Showroom — Mumbai", status: "Confirmed" },
];
        ...
        {appts.map((a, i) => (
          <div key={i} className="gradient-card glass rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground flex items-center gap-2 mt-1"><Clock className="w-3.5 h-3.5" />{a.date}</p>
              <p className="text-sm text-muted-foreground flex items-center gap-2 mt-0.5"><MapPin className="w-3.5 h-3.5" />{a.loc}</p>
            </div>
            <div className="flex items-center gap-3">
              <span className={`px-2.5 py-1 rounded-md text-xs font-semibold ${a.status === "Confirmed" ? "bg-success/15 text-success" : "bg-warning/15 text-warning"}`}>{a.status}</span>
